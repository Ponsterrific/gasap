<html>

	<head>
		<title>GASAP</title>

		<link rel="stylesheet" type="text/css" href="css/style.css">
		<link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.4/css/bootstrap.min.css">
		<link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/font-awesome/4.3.0/css/font-awesome.min.css">
		
	</head>

	<body>

		<div id="wrapper">
			<header>
				 <div class="row">
					<ul class="main-nav">
						
					</ul>
				</div>
				<div class="sub-header col-md-12">
					<div class="display-user-container">
						<a class="display-user" href=""><img src=""/>Hi User!</a>
					</div>
					<div class="user-account-container">
						<a class="user-logout" href=""><img src=""/>Logout</a>
					</div>
				</div>
			</header>
			
			<div id="content">
				<div class="container-fluid">
					<!--<div class="left-sidebar-container remove-padding col-md-2">
						<div class="left-sidebar "></div>
					</div>-->
					<div class="main col-md-10 col-xs-12">
						<div class="row">
							<ul class="menu-navigation hidden-xs">
								<li><a href="student_index.php"><i class="fa fa-user"></i>PROFILE</a></li>
								<li><a href="student_grades.php"  class="menu-selected"><i class="fa fa-list-alt"></i>GRADES</a></li>
								<li><a href="student_schedule.php"><i class="fa fa-calendar"></i>SCHEDULE</a></li>
								<li><a href="student_e-board.php"><i class="fa fa-clipboard"></i>E-BOARD</a></li>
								<li><a href="student_evaluation.php"><i class="fa fa-certificate"></i>EVALUATION</a></li>
							</ul>

							<div class="main-content box-shadow col-md-12 col-xs-12">
								<div class="content-block box-shadow col-md-12 col-xs-12">
									<div class="content-block-header"><h4>School Year</h4></div>
									
								</div>
								<!-- END content-block -->
								<div class="content-block box-shadow col-md-12 col-xs-12">
									<div class="content-block-header"><h4>Notes</h4></div>
									
								</div>
								<!-- END content-block -->
								<div class="content-block  box-shadow col-md-12 col-xs-12">
									<div class="content-block-header"><h4>---</h4></div>
								</div>
								<!-- END content-block -->
								<div class="row">
									<div class="col-md-6">
										
										<div class="content-block box-shadow col-md-12 col-xs-12">
											<div class="content-block-header"><h4>Years Attended</h4></div>
												
										</div>
									</div>
									
										<!-- END content-block -->
									<div class="col-md-6">
										<div class="content-block box-shadow col-md-12 col-xs-12">
											<div class="content-block-header"><h4>Deficiencies</h4></div>
												
										</div>
									</div>
								</div>
								<!-- END content-block -->
							</div>
						</div>
					</div>
					<div class="right-sidebar-container remove-padding box-shadow col-md-2 col-xs-12">
						<div class="notification-header col-md-12">
							<i class="fa fa-bell"></i>
							<h2>NOTIFICATIONS</h2>
						</div>
					</div>
				</div>
			</div>

	    </div>

		<script type="javascript" src="script.js"></script>
		<script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.4/js/bootstrap.min.js"></script>
	</body>

</html>